...
const NFT=()=>{
  ...
  console.log(tokenID)
  const marketplace=useMarketplace('0xAF1da844b72bBAB01d1F92B14CfAD25374c09c80');
...
const buyNFT=async()=>{
  ...
  return(
      ...
      <div className={style.nftContainer}>
        ...
        <div className={style.rightContainer}>
          ...
          <div className={style.buyoutContainer}>
           <NFTSalesInfo price={listing?.buyoutCurrencyValuePerToken?.displayValue} buttonLoading={buttonLoading} buyNFT={buyNFT} className='my-5'/>
    ...
    <div className="mt-5 uppercase text-center text-3xl font-semibold">recent Bids</div>       
<div className="overflow-x-auto relative border my-3 mb-8 rounded-lg mx-2">
    <table className="w-full text-sm text-left ">
        <thead className=" uppercase border-b">
            <tr>
                <th scope="col" className="py-3 px-6 text-center">
                   <div> bidder name</div><div></div>
                </th>
                <th scope="col" className="py-3 px-6 text-center">
                    price
                </th>
        ...
        <tbody>
            <tr className="border-b">
                <th scope="row" className="py-4 px-6 font-medium whitespace-nowrap text-gray-500 text-center">
                    {Names[Math.floor(Math.random()*Names.length)]}
                </th>
                <td className="py-4 px-6 text-gray-500 text-center">
                {(Math.random() * (listing?.buyoutCurrencyValuePerToken?.displayValue - 0 + 0.005) + 0).toFixed(3)}                </td>
                <td className="py-4 px-6 text-gray-500 text-center">
                    {randomDate(new Date(2022, 5, 1), new Date()).toString().slice(0,28)}
                </td>
                <td className="py-4 px-6 text-gray-500 text-center">
                {randomDate(new Date(2023, 5, 1), new Date()).toString().slice(0,28)}
                </td>
            </tr>
            <tr className="border-b">
                <th scope="row" className="py-4 px-6 font-medium whitespace-nowrap dark:text-white text-gray-500 text-center">
                {Names[Math.floor(Math.random()*Names.length)]}
                </th>
                <td className="py-4 px-6 text-gray-500 text-center">
                {(Math.random() * (listing?.buyoutCurrencyValuePerToken?.displayValue - 0 + 0.005) + 0).toFixed(3)}                </td>
            ...
            <tr className="">
                ...
                <td className="py-4 px-6 text-gray-500 text-center">
                {(Math.random() * (listing?.buyoutCurrencyValuePerToken?.displayValue - 0 + 0.005) + 0).toFixed(3)}
                </td>
                <td className="py-4 px-6 text-gray-500 text-center">
                {randomDate(new Date(2022, 5, 1), new Date()).toString().slice(0,28)}
                </td>
